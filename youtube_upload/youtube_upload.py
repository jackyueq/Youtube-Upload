...
def upload_video(youtube, options, video_path, total_videos, index):
    ...
    keywords = to_utf8(options.keywords)
...
def run_main(parser, options, args, output=sys.stdout):
    ...
    if options.create_playlist:
        create_playlist = to_utf8(options.create_playlist)
        title, description, private = tosize(create_playlist.split("|", 2), 3)
        ...
        playlist_id = playlist_uri.replace("?client=tokland-youtube_upload", "").replace("https://gdata.youtube.com/feeds/api/playlists/", "")
        output.write(playlist_id+"\n")
      ...
      for index, video_path in enumerate(args):
        url, video_id = upload_video(youtube, options, video_path, len(args), index)
        output.write(url + "\n")
        # Add uploaded video to playlist
        if options.add_to_current_playlist:
            debug("Adding video (%s) to playlist: %s" % (video_id, options.add_to_current_playlist))        
            youtube.add_video_to_playlist(video_id, options.add_to_current_playlist)
...
def main(arguments):
    ...
    parser.add_option('', '--add-to-current-playlist', dest='add_to_current_playlist', type="string", default=None,
        metavar="URI", help='Add video(s) to an existing playlist')
